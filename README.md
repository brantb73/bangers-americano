...
5. **Players** (mid-session): tap **Players** to add late arrivals (0 differential), **Sit** someone for this/next round (they stay in the session), or **Leave** to take them off the active roster for the rest of the night. Leave asks you to confirm — past results stay, marked left, and they are not scheduled again. Adding that same name (or **Return**) brings them back with their stats. A highlighted Sit is a bye — system or manual — tap again to unsit and sit someone else. You can’t drop below 4 active players. Unscored current rounds regenerate automatically; if scores are in, sit and leave apply next round.
6. **Swap who’s playing**: tap a name on a court card or the sit card, then tap who they should switch with — another court, the other team (switch partners), or someone sitting. **Cancel** clears the selection. Only unscored courts can change; a scored court stays locked (use **Edit score**). Sit-out counts and partner history follow the lineup that actually plays.
7. View **live placement** (wins, differential +/−, games played). Rank is **wins first, differential breaks ties**. Rank changes flash after scores. Settings stay available mid-session.
8. Optional **Switch to King’s Court** finish (confirm): Court 1 is King’s. After each game, winners move up one court, losers move down one; Court 1 winners and Court N losers stay. Partners **split** (teammates become opponents). Seed from current standings (wins, then differential) or shuffle. Same game-to / win-by / sudden death scoring; differential keeps accumulating and King’s Court wins show on the board. Sit-outs rotate at the bottom courts. A swap only changes this game’s court; the next ladder uses that result.
9. **End session** for a medal-style final placement. The session is **archived to history by date**. **Undo** reverses the last score entry. The recap mentions a King’s Court finish when you used one.
...
During an active Americano session, tap **Switch to King’s Court**, confirm the seed (standings by default, or random), and play the ladder. Mid-session add, sit, leave, and lineup swaps work the same way as Americano. Leave still needs at least 4 active players; an unscored round rebuilds without them, and if scores are already in they drop out next round (the ladder fills the open spot from the bench). A swap changes who plays that court for this game only — movement after the score uses the real winners and losers. Rematch returns to Americano setup.
